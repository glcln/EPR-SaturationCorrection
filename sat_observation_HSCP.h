...
#ifndef sat_observation_HSCP_h
#define sat_observation_HSCP_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   sat_observation_HSCP(TTree *tree=0);
   virtual ~sat_observation_HSCP();
...
#ifdef sat_observation_HSCP_cxx
sat_observation_HSCP::sat_observation_HSCP(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ROOT_Files/nt_HSCPgluino_2TeV.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ROOT_Files/nt_HSCPgluino_2TeV.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("ROOT_Files/nt_HSCPgluino_2TeV.root:/stage");
...
sat_observation_HSCP::~sat_observation_HSCP()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t sat_observation_HSCP::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t sat_observation_HSCP::LoadTree(Long64_t entry)
{
...
void sat_observation_HSCP::Init(TTree *tree)
{
...
Bool_t sat_observation_HSCP::Notify()
{
...
void sat_observation_HSCP::Show(Long64_t entry)
{
...
Int_t sat_observation_HSCP::Cut(Long64_t entry)
{
...
#endif // #ifdef sat_observation_HSCP_cxx
